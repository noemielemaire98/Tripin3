...
class DetailsHotel : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val databaseoffers =
            Room.databaseBuilder(this, AppDatabase::class.java, "offersDatabase")
                .build()
        ...
        GlobalScope.launch {
            runBlocking { offerDao!!.deleteOffers() }

            val hotelId = hotel!!.hotelId
            var offer : String? = null

            Log.d("resultOffer",hotel!!.listIdOffer.toString())
            val resultOffers =amadeus.shopping.hotelOffersByHotel[Params.with("hotelId", hotel!!.hotelId)]
        ...
        detail_hotel_telephone_texview.setTypeface(null, Typeface.ITALIC)
            detail_hotel_adresse_texview.text = "${hotel?.adresse?.get(0)} ${hotel?.adresse?.get(1)}, ${hotel?.adresse?.get(2)}, ${hotel?.adresse?.get(3)}, ${hotel?.adresse?.get(4)}"
